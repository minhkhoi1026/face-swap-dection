...
class AbstractMetricEvaluator(abc.ABC):
    @abc.abstractmethod
...
class SHRECMetricEvaluator(AbstractMetricEvaluator):
    def __init__(self, embed_dim) -> None:
        self.retriever = FaissRetrieval(dimension=embed_dim, cpu=True)
    ...
    def append(self, g_emb, q_emb, query_ids, gallery_ids, target_ids):
        self.gallery_embeddings.append(g_emb.detach().cpu().numpy())
        self.query_embeddings.append(q_emb.detach().cpu().numpy())
        self.query_ids.extend(query_ids)
        self.gallery_ids.extend(gallery_ids)
    ...
    def evaluate(self):
        max_k = len(self.gallery_ids)  # retrieve all available gallery items
        query_embeddings = np.concatenate(self.query_embeddings, axis=0)
        gallery_embeddings = np.concatenate(self.gallery_embeddings, axis=0)
        ...
        model_labels = np.array(encode_labels(self.query_ids), dtype=np.int32)
        rank_matrix = top_k_indexes_all
        return evaluate(rank_matrix, model_labels, model_labels)


def evaluate_submission(
    csv_path: os.PathLike,
    gt_path: os.PathLike,
    query_path: os.PathLike,
    model_path: os.PathLike,
    subset_path: os.PathLike,
):
    ...
    text_col = (
        gt["Text Query ID"] if "Text Query ID" in gt.columns else gt["Sketch Query ID"]
    )
    ...
    if len(submission) != num_tot_queries or len(submission.columns) != num_models + 1:
        return return_error(
            f"Submission must have shape (num_queries, num_models + 1)! \n"
...
def main():
    print(
        evaluate_submission(
            ...
            "SketchQuery_GT_Train.csv",
            "SketchQuery_Train.csv",
            "References.csv",
            "SketchQueryID_TrainTrain.csv",
        )
    )

...
class Walk(nn.Module):
    ...
    def __init__(self, in_channel, k, curve_num, curve_length):
        ...
        self.agent_mlp = nn.Sequential(
            nn.Conv2d(in_channel * 2, 1, kernel_size=1, bias=False), nn.BatchNorm2d(1)
        )
        self.momentum_mlp = nn.Sequential(
            nn.Conv1d(in_channel * 2, 2, kernel_size=1, bias=False), nn.BatchNorm1d(2)
        )
    ...
    def forward(self, xyz, x, adj, cur):
        ...
        batch_offset = torch.arange(0, bn).detach() * tot_points

        # indices of neighbors for the starting points
        tmp_adj = (adj + batch_offset.view(-1, 1, 1).to(device)).view(
            adj.size(0) * adj.size(1), -1
        )  # bs, n, k

        # batch flattened indices for teh starting points
        flatten_cur = (cur + batch_offset.view(-1, 1, 1).to(device)).view(-1)
        ...
        for step in range(self.curve_length):
            ...
            if step == 0:
                ...
                pre_feature = starting_points.view(bn, self.curve_num, -1, 1).transpose(
                    1, 2
                )  # bs * n, c
            else:
                # dynamic momentum
                ...
                cat_feature = torch.cat(
                    (cur_feature, pre_feature), dim=-1
                )  # bs, c, n, 2

                # update curve descriptor
                pre_feature = torch.sum(
                    cat_feature * att_feature, dim=-1, keepdim=True
                )  # bs, c, n
                pre_feature_cos = (
                    pre_feature.transpose(1, 2)
                    .contiguous()
                    .view(bn * self.curve_num, -1)
                )
            ...
            cur_feature = torch.sum(
                pick_values * pre_feature_expand, dim=-1, keepdim=True
            )  # bs, c, n, 1

            cur_feature_cos = (
                cur_feature.transpose(1, 2).contiguous().view(bn * self.curve_num, c)
            )
...
def index_points(points, idx):
    ...
    repeat_shape[0] = 1
    batch_indices = (
        torch.arange(B, dtype=torch.long).to().view(view_shape).repeat(repeat_shape)
    )
...
def farthest_point_sample(xyz, npoint):
    ...
    farthest = torch.randint(0, N, (B,), dtype=torch.long).to() * 0
...
def query_ball_point(radius, nsample, xyz, new_xyz):
    ...
    group_idx = torch.arange(N, dtype=torch.long).to().view(1, 1, N).repeat([B, S, 1])
...
class LPFA(nn.Module):
    ...
    def group_feature(self, x, xyz, idx):
        ...
        idx_base = (
            torch.arange(0, batch_size).view(-1, 1, 1).to(idx.device) * num_points
        )
        idx = idx + idx_base
        idx = idx.view(-1)
        ...
        point_feature = point_feature.view(
            batch_size, num_points, self.k, -1
        )  # bs, n, k, 3
        points = xyz.view(batch_size, num_points, 1, 3).expand(
            -1, -1, self.k, -1
        )  # bs, n, k, 3

        point_feature = (
            torch.cat((points, point_feature, point_feature - points), dim=3)
            .permute(0, 3, 1, 2)
            .contiguous()
        )
    ...
    def __init__(
        ...
        if self.use_curve:
            ...
            self.curvegrouping = CurveGrouping(
                planes, k, curve_config[0], curve_config[1]
            )
...
class CurveAggregation(nn.Module):
    ...
    def forward(self, x, curves):
        ...
        curve_features = (
            torch.cat((x_inter, x_intra), dim=-1).transpose(1, 2).contiguous()
        )
...
class CurveNet(nn.Module):
    def __init__(self, num_classes=2, k=20, setting="default"):
